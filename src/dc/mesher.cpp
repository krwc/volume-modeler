...
Mesher::Mesher(const shared_ptr<ComputeContext> &compute_ctx)
    ...
    m_edges_scan = move(Scan(compute_ctx->queue, num_edges));
    m_edge_mask =
            compute::vector<uint32_t>(num_edges, compute_ctx->context);
    m_scanned_edges =
            compute::vector<uint32_t>(num_edges, compute_ctx->context);
    ...
    m_voxels_scan = move(Scan(compute_ctx->queue, num_voxels));
    m_voxel_mask =
            compute::vector<uint32_t>(num_voxels, compute_ctx->context);

    m_scanned_voxels =
            compute::vector<uint32_t>(num_voxels, compute_ctx->context);
    compute::fill(m_voxel_mask.begin(), m_voxel_mask.end(), 0,
                  compute_ctx->queue);
    compute::fill(m_scanned_voxels.begin(), m_scanned_voxels.end(), 0,
                  compute_ctx->queue);

    m_voxel_vertices =
            compute::vector<float>(3 * num_voxels, compute_ctx->context);
...
void Mesher::enqueue_get_active_edges(Chunk &chunk) {
    /* Mark active edges as active */
    m_select_active_edges.set_arg(0, m_edge_mask);
    m_select_active_edges.set_arg(1, chunk.samples);

    const size_t size[3] = { N + 3, N + 3, N + 3};

    auto event = m_compute_ctx->queue.enqueue_nd_range_kernel(
            m_select_active_edges, 3, nullptr, size, nullptr);

    /* Count them */
    m_edges_scan
            .inclusive_scan(m_edge_mask, m_scanned_edges, m_compute_ctx->queue,
                            event)
            .wait();
}
...
void Mesher::enqueue_solve_qef(Chunk &chunk) {
    ...
    static const size_t size[3] = { VM_CHUNK_SIZE + 2,
                             ...
                             VM_CHUNK_SIZE + 2 };
    auto event =
            m_compute_ctx->queue.enqueue_nd_range_kernel(m_solve_qef, 3,
                                                         nullptr, size, nullptr);
    event.wait();
    /* Count active voxels */
    m_voxels_scan.inclusive_scan(m_voxel_mask, m_scanned_voxels,
                                 m_compute_ctx->queue, event).wait();
}
...
void Mesher::contour(Chunk &chunk) {
    enqueue_get_active_edges(chunk);
