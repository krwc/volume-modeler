...
kernel void select_active_edges(global uint *out_selection,
    ...
    /* Ignore edges from additional layer */
    if (x < 1 || y < 1 || z < 1) {
        return;
    }
    if (x > DIM_VOXEL_GRID + 1 || y > DIM_VOXEL_GRID + 1
        || z > DIM_VOXEL_GRID + 1) {
        return;
    }
    /**
     ...
     *
     * Oh, and by the way, the `out_selection` array is (N+3)^3 (even though the
     ...
     */
    const uint index = 3 * (x + DIM_SAMPLES * (y + DIM_SAMPLES * z));
